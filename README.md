...
AIS 원본 데이터
  ...
  → 이상탐지 4종
      ...
      ├─ 항로이탈 (waypoint + 실제 이동구간(corridor) 기반, BallTree/cross-track distance)
...
| 항로 이탈 (corridor 기반, 15km 기준) | 8,956건 (약 5.1%, 점 기반 대비 22% 감소) |
...
- 급변침로/속도 z-score는 실제로는 선박종류 세부코드(60/61/65/69 등) 기준으로 그룹핑되지만,
  세부코드별 표본이 매우 적은 경우(1~3척)가 많아 결국 전체 데이터 기준으로 자동 대체(fallback)
  되는 경우가 잦음 — "대분류로 뭉쳐서 부정확"한 게 아니라 "세분류인데 표본이 부족"한 문제

### 🔺 waypoint(DBSCAN 점 클러스터링)의 근본적 한계 — corridor로 부분 개선
...
- **왜 그래도 어느 정도 말이 되는가**: 목적지가 달라도 항구 입구·좁은 해협·정박지처럼 대부분의
  ...
  샌프란시스코행이든 다 지나감).
- **개선함**: waypoint 사이를 실제로 오간 선박들의 이동 순서를 학습해 "corridor(이동 구간)"를
  만들고, 그 구간까지의 수직 거리(cross-track distance)를 고려하도록 개선했습니다
  (`build_corridor_edges`, `detect_route_deviation_corridor`). 결과적으로 오탐이 22%
  감소했습니다 (11,499건 → 8,956건).
- **여전히 남는 한계**: corridor는 "최소 2척 이상이 실제로 지나간 구간"만 학습하기 때문에,
  학습 범위 밖(예: 원양, 드문 항로)의 정상 항해는 여전히 오탐될 수 있습니다. 실제로 검증
  과정에서 특정 화물선 사례(MMSI 210185000)는 corridor 개선 후에도 여전히 이탈로 잡혔는데,
  이 배가 학습된 13개 corridor 범위 밖(만 바깥쪽)을 항해했기 때문으로 추정됩니다.
- **더 정확히 하려면**: (1) 선박별 궤적 자체를 DTW 등으로 유사도 클러스터링, (2) TREAD 논문처럼
  더 정교한 방향성 흐름(flow) 추출, (3) 격자 기반 밀도맵 + 방향정보 저장.
...
- [x] 선박종류 z-score 그룹핑 검증 — 실제로는 표시용 이름("여객선")이 아닌 **세부 코드(60/61/65/69 등)
      기준으로 이미 그룹핑되고 있음**을 확인 (단, 세부코드별 표본이 적으면 전체 통계로 자동 폴백)
- [x] 항로이탈을 corridor(waypoint 간 실제 이동구간) 기반으로 개선 — 오탐 22% 감소 (11,499→8,956건),
      다만 학습된 corridor 범위 밖(원양 등) 항해는 여전히 오탐 가능 (완전 해결은 아님)
